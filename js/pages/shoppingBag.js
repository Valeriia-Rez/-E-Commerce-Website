...
const checkoutItemsFromShoppingBag = () => {
    ...
    const checkoutMessage = document.querySelector(
        "[data-selector='checkout_message']"
    );
...
const renderShoppingBagItems = () => {
    const { items } = storage.getShoppingCart();
    const shoppingBagItems = document.querySelector(
        "[data-selector='shopping_bag_items']"
    );
    ...
    <div class="product_item shopping_bag_item d-flex flex-direction-row mb-2">
        ...
        <div class="product_info d-flex flex-direction-column align-items-baseline pl-4 f-size-14-tablet">
            ...
            <span class="product_price"><strong>£ ${
            ...
            <div class="product_details mt-2 f-size-10 f-size-12-tablet">
                ...
                <p>
                    ...
                    <button data-id="${
                      item.storageId
                    }" data-selector="decrease_quantity_button" class="text-danger f-size-16">&#8722;</button>
                    <span class="pl-2 pr-2">${item.quantity}</span>
                    <button data-id="${
                      item.storageId
                    }" data-selector="increase_quantity_button" class="text-danger f-size-16">+</button>
                </p>
            </div>
            <button data-id="${
              item.storageId
            }" data-selector="remove_item_button" class="btn_link text-danger f-size-11 f-size-13-tablet">Remove item</button>
        </div>
    </div>
        `
        )
...
const renderCheckoutSection = () => {
        const { totalCost, discount } = storage.getShoppingCart();
        const checkoutArea = document.querySelector(
            "[data-selector='checkout_area']"
        );
        checkoutArea.innerHTML = `
        <div class="checkout_discount_amounts text-center mb-4">
            ${
              discount
                ? `<span class="applied_discount text-danger f-size-16-tablet">Applied discount: £${discount.toFixed(
                    2
                  )}</span>`
                : ""
            }
            <div class="price_after_discount mt-2">
                <span class="f-size-16 f-size-20-tablet f-size-22-desktop">Total price: ${
  ...
  const clearBagButton = document.querySelector(
    "[data-selector='clear_bag_button']"
  );
  const checkoutButton = document.querySelector(
    "[data-selector='checkout_button']"
  );
